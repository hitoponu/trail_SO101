...
class ReachToPoint(Node):
    def __init__(self) -> None:
        ...
        defaults = {
            ...
            "stow_positions": [0.0, -1.4, 1.4, 0.6, 0.0],
        ...
        if self._require_stationary:
            self.create_subscription(
                Odometry, str(param("odom_topic")), self._odom_cb, 10
            )
    ...
    def _handle_target(self, message: PoseStamped) -> None:
        ...
        if self._require_stationary and self._odom_twist is not None:
    ...
    def _watch_base(self) -> None:
        ...
        if not self._busy or self._plan_translation is None:
            return
