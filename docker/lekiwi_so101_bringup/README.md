...
> ★ **統合スタックを使うなら [`../../docs/interfaces.md`](../../docs/interfaces.md) を見てください。**
> こちらは 4 コンテナ構成の前置き（`$E` / `$N` の 2 種類）で書かれており、
> 「`nav2_msgs` はベースコンテナにしか入っていない」など**統合後は当てはまらない**
> 注記を含みます。
...
> D435i は約 72g、マウントとケーブルで 100g 級。アームは約 750g で、
> しかも全関節 `P=16` で保持力が弱いという未解決問題がある。
> 起動時に一瞬トルクが抜けるので、先端重量が増えた状態で落ちる。
>
...
★ **手首カメラの取付姿勢の既定値は D405 + 公式ホルダ前提の幾何計算値**で、
D435i には SO-101 用の公式マウントが存在しない。実測して `.env` の
`WRIST_CAMERA_*` を上書きするまで、点群の絶対位置は信用しないこと。
