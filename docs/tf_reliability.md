...
| # | 区間 | 由来 | 信頼度 | 影響 |
| --- | --- | --- | --- | --- |
| 1 | `arm_gripper_link → wrist_camera_mount_link → wrist_camera_link` | **床点3点×2姿勢による二段補正**（レンズ中心は未実測） | 中 | 点群が丸ごとずれる |
...
| 6 | `base_footprint → base_link` | **下段プレート上面の実測** | 中〜高 | 小さい |
...
wrist_camera_mount_link → wrist_camera_link  xyz=(-0.00097,-0.06748,0.03586)
                                             rpy=(3.03341, 1.26087, 1.46705)
```

旧値は D405 + 公式 `Wrist_Roll_D405_Holder` を前提に STL から幾何計算したものだった。
現在値は、実機で取得した床上3点から床面が水平になるように補正したD435iの姿勢である。
手元のカメラは **D435i** で、**SO-101 用の公式マウントが存在しない**
（公式 D435 マウントは SO-100 専用で SO-101 の手首に受け穴が無い）。

→ **床面に対する姿勢と高さは補正済みだが、レンズ中心と実マウントの絶対位置は未実測である。**
引き続きRVizで点群と床面の一致を確認する。
...
`base_link` は下段プレート上面に置き、地面からの高さを実測値 `0.040 m` とした。
プレート厚は `0.007 m` なので、下段プレートは `z=0.033〜0.040 m` にある。
車輪半径 `0.05 m` から車軸高さを `0.010 m` と導出し、車輪の接地も維持する。

上段プレートは `base_link` から `0.052 m` 上に置く。したがって上面は
`z=0.092 m`、下面は `z=0.085 m` になる。LiDAR の高さは新しい `base_link` 基準で
`0.040 + 0.030 = 0.070 m` になる。
